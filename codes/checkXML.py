...
def check(answer, rightAnswer):
    ...
    print(countMaxPoints(answer, rightAnswer))
    pt=(countPoints(answer, rightAnswer)/countMaxPoints(answer, rightAnswer))*100
    if pt < 0: pt = 0
    return '{:.2f}%'.format(pt)
